...
import { Card, Form, Button, Spinner, Nav, Badge, Dropdown } from 'react-bootstrap';
...
const ChatBubble = () => {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (chatBubbleRef.current && !chatBubbleRef.current.contains(event.target) && isOpen) {
        setIsOpen(false);
      }
    }
  ...
  const fetchConversations = async () => {
    try {
      const apiUrl = `${window.location.protocol}//${window.location.hostname}:5001/api/assistant/conversations`;
      ...
      } else if (result.data.conversations?.length === 0) {
  ...
  const loadConversation = async (conversationId) => {
    try {
      setLoading(true);
      const apiUrl = `${window.location.protocol}//${window.location.hostname}:5001/api/assistant/conversations/${conversationId}`;
      const result = await axios.get(apiUrl);
  ...
  const askAssistant = async (questionText = null) => {
    ...
    try {
      ...
      const apiUrl = `${window.location.protocol}//${window.location.hostname}:5001/api/assistant`;
  ...
  return (
    <div className="chat-bubble-container" ref={chatBubbleRef}>
      ...
      {/* The chat window */}
      {isOpen && (
        <Card className="chat-popup-window">
